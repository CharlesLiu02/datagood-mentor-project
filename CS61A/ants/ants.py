...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        checker = self.place
        distance = 0
        while checker != beehive:
            if distance > self.max_range:
                return
            if distance < self.min_range:
                distance = distance
            elif checker.bees != []:
                return rANTdom_else_none(checker.bees)

            checker = checker.entrance
            distance += 1
        return
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor:
            lst = self.place.bees.copy()
            for bee in lst:
                Insect.reduce_armor(bee, self.damage + amount)
            self.place.bees = [x for x in lst if x.place != None]
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.number > 1:
            ...
            checker = self.place
            while checker:
                if checker.ant and checker.ant.buffed == False:
                    checker.ant.damage += checker.ant.damage
                    checker.ant.buffed = True
