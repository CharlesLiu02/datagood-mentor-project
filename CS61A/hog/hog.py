...
def swine_align(player_score, opponent_score):
    ...
    def gcd(score,list):
        number_counter = 1
        while number_counter <= score:
            if score % number_counter == 0:
                list.append(number_counter)
            number_counter += 1
        return

    gcd(player_score,player_factors)
    gcd(opponent_score, opponent_factors)
    for a in opponent_factors:
        for b in player_factors:
            if a == b:
                common_factors.append(a)
    if common_factors[-1] >= 10:
        return True
    else:
        return False
    # END PROBLEM 4a
...
def pig_pass(player_score, opponent_score):
    ...
    if player_score >= opponent_score:
        return False
    elif abs(player_score - opponent_score) < 3:
        return True
    else:
        return False
    # END PROBLEM 4b
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            if not extra_turn(score0, score1):
                who = other(who)
            if score0 > score1:
                announce_lead_changes(last_leader=None)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def highest(score0, score1):
        if who == 0:
            ...
            return announce_highest(who, last_score=score0, running_high = max(difference,running_high))
        else:
            difference = score1 - last_score
            if difference > running_high:
                print(difference, "point(s)! The most yet for Player", who)
            return announce_highest(who, last_score=score1, running_high = max(difference, running_high))
    return highest
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    number_counter = 0
    while number_counter < trials_count:
        result = 0
